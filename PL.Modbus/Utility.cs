...
namespace PL.Modbus
{
    internal static class Utility
    {
        ...
        public static ushort Crc(byte[] buffer, int byteCount)
        {
            ushort crc = 0xFFFF;
            for (int i = 0; i < byteCount; i++)
            {
                byte crcTableIndex = (byte)((crc & 0xFF) ^ (buffer[i]));
                crc >>= 8;
                crc ^= _crcTable[crcTableIndex];
            }
            return crc;
        }

        public static byte Lrc(byte[] buffer, int byteCount)
        {
            byte lrc = 0;
            for (int i = 0; i < byteCount; i++)
                lrc += buffer[i];
        ...
        public static byte[] AsciiStringToByteArray(string asciiString)
        {
            if (asciiString.Length % 2 != 0)
                throw new ArgumentException("ASCII buffer length should be even.");
            
            byte[] array = new byte[asciiString.Length / 2];
            for (int i = 0; i < array.Length; i++)
                array[i] = Convert.ToByte(asciiString.Substring(i * 2, 2), 16);
            return array;
        }
